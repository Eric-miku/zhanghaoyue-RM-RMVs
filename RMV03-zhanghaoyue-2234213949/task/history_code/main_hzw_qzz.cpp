...
void fitAngleSpeed(const std::vector<double>& times, const std::vector<double>& speeds, double &A, double &omega, double &phi, double &b, double &c) {
    ...
    cv::Mat plot = cv::Mat(720, 1080, CV_8UC3, cv::Scalar(0, 0, 0));
    for (size_t i = 0; i < times.size(); ++i) {
        // 为每个数据点添加残差块
        cv::circle(plot,Point((int)(times[i]*200),(int)(speeds[i]*20+100)),1,cv::Scalar(0,255,0),-1);
...
int main()
{
    ...
    for(int num=0;num<50;num++)
    {
        ...
        while (1)
        {
            ...
            if(cooldown<=0 && angle < -3 && old_angle > 3){
                advanced+=pi*2;
                cooldown=10;
            }
            if(cooldown>0){
            ...
            old_angle=angle;
            if(cooldown>0 && angle > 3){
                angles.push_back(angle+advanced-2*pi);
            }else{
                angles.push_back(angle+advanced);
            }
