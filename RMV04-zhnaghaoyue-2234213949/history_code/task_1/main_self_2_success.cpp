...
void image_process_hsv(Mat& image){
    ...
    cv::Scalar lowerRed1(35, 43, 0); //蓝色
    cv::Scalar upperRed1(100, 255, 255);
    cv::inRange(hsvImage, lowerRed1, upperRed1, mask1); // 创建二值化图像1

    cv::Scalar lowerRed2(0, 0, 0); // 白色
    cv::Scalar upperRed2(255, 255, 255);
    cv::inRange(hsvImage, lowerRed2, upperRed2, mask2); // 创建二值化图像2

    // 合并两个掩码
    cv::bitwise_or(mask1, mask2, mask);
    cv::Scalar lowerBlue1(35, 43, 200); 
    cv::Scalar upperBlue1(100, 255, 255);
    cv::inRange(hsvImage, lowerBlue1, upperBlue1, mask); // 创建二值化
...
void image_process_RGB(Mat& image){
    // 2. 转换颜色空间为 RGB
    cv::Mat rgbImage;
